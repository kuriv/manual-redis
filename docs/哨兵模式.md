...
哨兵模式是在主从复制的基础上进行监控，修改 `sentinel.conf` 配置文件内容如下。
...
sentinel monitor sentinel redis-master 6379 2
...
# Example:
#
sentinel auth-pass sentinel foobared
...
```
SENTINEL REPLICAS sentinal
```

执行下面的命令，检查哨兵法定人数是否满足。

```
SENTINEL CKQUORUM sentinel
```

执行下面的命令，手动触发故障转移。

```
SENTINEL FAILOVER sentinel
```
